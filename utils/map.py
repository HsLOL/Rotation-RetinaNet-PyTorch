...
def eval_mAP(gt_root_dir=None, test_path=None, eval_root_dir=None, use_07_metric=False, thres=0.5):
    """
    Args:
    ...
    GT_PATH = os.path.join(gt_root_dir,  test_path)
    DR_PATH = os.path.join(eval_root_dir,  'detection-results')
    # if there are no images then no animation can be shown
    IMG_PATH = os.path.join(gt_root_dir, 'images-optional')
    ...
    """
    TEMP_FILES_PATH = os.path.join(eval_root_dir, ".temp_files")
    if not os.path.exists(TEMP_FILES_PATH): # if it doesn't exist already
        os.makedirs(TEMP_FILES_PATH)
    output_files_path = os.path.join(eval_root_dir, "output")
    if os.path.exists(output_files_path): # if it exist already
        # reset the output directory
        shutil.rmtree(output_files_path)

    os.makedirs(output_files_path)
    if draw_plot:  # plot some curves
    ...
    gt_counter_per_class = {}  # save the number of per ground-truth
    ...
    with open(output_files_path + "/output.txt", 'w') as output_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            """
            nd = len(dr_data)  # the number of the detections
            tp = [0] * nd  # creates an array of zeros of size nd
            fp = [0] * nd
            # print('evaluate on class:   {}  '.format(class_name))
            # for idx, detection in enumerate(tqdm(dr_data)):
            for _index, detection in enumerate(dr_data):  # todo: idx -> _index
                ...
                # as3;fmtsign detection-results to ground truth object if any
                ...
                for idx, obj in enumerate(ground_truth_data):
