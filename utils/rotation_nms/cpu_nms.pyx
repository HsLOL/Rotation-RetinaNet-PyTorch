...
def cpu_nms(
    ...
    cdef np.ndarray[np.float32_t, ndim=1] ws = dets[:, 2]
    cdef np.ndarray[np.float32_t, ndim=1] hs = dets[:, 3]
    cdef np.ndarray[np.float32_t, ndim=1] xx = dets[:, 0]
    cdef np.ndarray[np.float32_t, ndim=1] yy = dets[:, 1]
